...
class DataIngestionPipeline:
    ...
    def process_dataset(
        ...
        Args:
            chunking_strategies: List of chunking strategies to apply
            eval_samples: Optional list of eval samples with eval_id. If provided, only these are processed.
            limit: Number of documents to process if eval_samples not provided

        Returns:
            Dictionary mapping collection names to total chunks inserted
        """
        logger.info(
            f"Starting data ingestion pipeline with {len(chunking_strategies)} strategies"
        )

        # Load dataset - either eval samples or train subset
        if eval_samples is not None:
            dataset = eval_samples
            logger.info(f"Processing {len(dataset)} eval samples")
        else:
        ...
        collection_stats = {
            strategy.get_collection_name(): 0 for strategy in chunking_strategies
        }

        for idx, document in enumerate(dataset):
            logger.info(f"Processing document {idx + 1}/{len(dataset)}")
            self._process_document(document, chunking_strategies, collection_stats)
    ...
    def _process_document(
        ...
        # Extract document_id and category from document
        # For eval samples: use eval_id, otherwise use original_index or fallback to enumeration
        document_id = document.get("eval_id") or str(
            document.get("original_index", "unknown")
        )
        ...
        for strategy in chunking_strategies:
            ...
            # Chunk the document with category metadata
            chunks = strategy.chunk_document(context, document_id, category=category)
            ...
            if chunks:
                ...
                logger.warning(
                    f"No chunks generated for document {document_id} with {collection_name}"
                )
